...
```php
$complianceController = $client->getComplianceController();
```
...
```php
function listIdentityChecks(string $userToken): IdvCheckListResult
```
...
try {
    $result = $complianceController->listIdentityChecks($userToken);
    echo 'IdvCheckListResult:';
    var_dump($result);
...
```php
function retrieveIdentityCheck(string $userToken, string $idvcToken): IdvCheckResult
```
...
$idvcToken = 'idvc-6e7567e0-c2db-485d-896d-45901a10aaaa';

$complianceController = $client->getComplianceController();

try {
    $result = $complianceController->retrieveIdentityCheck(
        $userToken,
        $idvcToken
    );
    echo 'IdvCheckResult:';
