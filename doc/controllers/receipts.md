...
try {
    $result = $receiptsController->listAccountReceipts(
        $accountToken,
        $page,
        $pageSize,
        $filter,
...
try {
    $result = $receiptsController->listPrepaidCardReceipts(
        $userToken,
        $destinationToken,
        $page,
        $pageSize,
...
try {
    $result = $receiptsController->listUserReceipts(
        $userToken,
        $page,
        $pageSize,
        $filter,
...
```php
function retrieveAccountReceipt(string $accountToken, string $receiptToken): ReceiptResult
```
...
|  --- | --- | --- | --- |
| `accountToken` | `string` | Template, Required | Auto-generated unique identifier representing a company account, prefixed with `acct-`.<br><br>**Constraints**: *Minimum Length*: `41`, *Maximum Length*: `41`, *Pattern*: `^acct-[0-9A-Fa-f]{8}(?:-[0-9A-Fa-f]{4}){3}-[0-9A-Fa-f]{12}$` |
| `receiptToken` | `string` | Template, Required | Auto-generated unique identifier representing a receipt, prefixed with `rcpt-`.<br><br>**Constraints**: *Pattern*: `^rcpt-[0-9A-Fa-f]{8}(?:-[0-9A-Fa-f]{4}){3}-[0-9A-Fa-f]{12}$` |
...
try {
    $result = $receiptsController->retrieveAccountReceipt(
        $accountToken,
        $receiptToken
    );
    echo 'ReceiptResult:';
...
```php
function retrievePrepaidCardReceipt(
    string $userToken,
    string $destinationToken,
    string $receiptToken
): ReceiptResult
...
try {
    $result = $receiptsController->retrievePrepaidCardReceipt(
        $userToken,
        $destinationToken,
        $receiptToken
    );
...
```php
function retrieveUserReceipt(string $userToken, string $receiptToken): ReceiptResult
```
...
try {
    $result = $receiptsController->retrieveUserReceipt(
        $userToken,
        $receiptToken
    );
    echo 'ReceiptResult:';
