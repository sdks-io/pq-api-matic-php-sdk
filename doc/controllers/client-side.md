...
```php
$clientSideController = $client->getClientSideController();
```
...
| `format` | [`string(PrepaidCardDataType)`](../../doc/models/prepaid-card-data-type.md) | Query, Required | Desired format for the prepaid card data. |
| `side` | [`?string(PrepaidCardImageSide)`](../../doc/models/prepaid-card-image-side.md) | Query, Optional | Side to specify when retrieving a prepaid card's image data. *Required if IMAGE format specified. |
...
try {
    $result = $clientSideController->createCardDataToken(
        $userToken,
        $destinationToken,
        $format,
        $side
    );
    echo 'PrepaidCardDataTokenResult:';
    var_dump($result);
...
```php
function createCardPINToken(string $userToken, string $destinationToken): PrepaidCardPinTokenResult
```
...
try {
    $result = $clientSideController->createCardPINToken(
        $userToken,
        $destinationToken
    );
    echo 'PrepaidCardPinTokenResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
} catch (ApiException $exp) {
...
$body = RetrieveCardDataBuilder::init()
    ->token('')
    ->build();
...
try {
    $result = $clientSideController->retrieveCardData(
        $userToken,
        $destinationToken,
        $format,
        null,
...
$body = RetrieveCardPinBuilder::init()
...
try {
    $result = $clientSideController->retrieveCardPIN(
        $userToken,
        $destinationToken,
        $body
    );
    echo 'PrepaidCardPinResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
} catch (ApiException $exp) {
...
$body = UpdateCardPinBuilder::init()
...
try {
    $result = $clientSideController->updateCardPIN(
        $userToken,
        $destinationToken,
        $body
    );
