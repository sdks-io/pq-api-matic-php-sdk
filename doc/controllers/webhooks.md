...
```php
function createSubscription(?CreateWebhookRequest $body = null): WebhookSubscriptionResult
```
...
```php
$body = CreateWebhookRequestBuilder::init()
    ->namespace(WebhookNamespaces::PAYMENTQUOTES_ACCEPTED)
...
try {
    $result = $webhooksController->createSubscription($body);
    echo 'WebhookSubscriptionResult:';
    var_dump($result);
...
```php
function deleteSubscription(string $webhookToken): WebhookSubscriptionResult
```
...
| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `webhookToken` | `string` | Template, Required | Auto-generated unique identifier representing a webhook subscription, prefixed with `webh-`.<br><br>**Constraints**: *Pattern*: `^webh-[0-9A-Fa-f]{8}(?:-[0-9A-Fa-f]{4}){3}-[0-9A-Fa-f]{12}$` |
...
```php
$webhookToken = 'webh-6260c132-5cb1-4e30-8b08-9ce559893acb';

$webhooksController = $client->getWebhooksController();

try {
    $result = $webhooksController->deleteSubscription($webhookToken);
    echo 'WebhookSubscriptionResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
...
try {
    $result = $webhooksController->listSubscriptions(
        $page,
        $pageSize,
        $filter,
        $sort,
    ...
    echo 'WebhookSubscriptionListResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
} catch (ApiException $exp) {
...
```php
function retrieveSubscription(string $webhookToken): WebhookSubscriptionResult
```
...
try {
    $result = $webhooksController->retrieveSubscription($webhookToken);
    echo 'WebhookSubscriptionResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
...
```php
function updateSubscription(?CreateWebhookRequest $body = null): WebhookSubscriptionResult
```
...
try {
    $result = $webhooksController->updateSubscription($body);
    echo 'WebhookSubscriptionResult:';
    var_dump($result);
} catch (ApiErrorResultException $exp) {
    echo 'Caught ApiErrorResultException:', $exp;
