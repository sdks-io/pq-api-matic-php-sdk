...
try {
    $result = $programController->listPrograms(
        $page,
        $pageSize,
        $filter,
        $sort,
...
```php
function retrieveProgram(string $programToken, ?string $language = null): ProgramResult
```
...
try {
    $result = $programController->retrieveProgram(
        $programToken,
        $language
    );
    echo 'ProgramResult:';
